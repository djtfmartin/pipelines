...
public class HTTPServer implements Closeable {
  ...
  private HTTPServer(
    ...
    registerHandler(
        "/help", defaultHandler == null ? new DefaultHandler() : defaultHandler, authenticator);
    registerHandler("/metrics", new MetricsHandler(config, registry), authenticator);
    registerHandler("/", new MetricsHandler(config, registry), authenticator);
  ...
  public static class Builder {
     ...
     * the randomly selected port by calling {@link HTTPServer#getPort()}.
     */
     ...
     * {@link #inetAddress(InetAddress)}. Default is empty, indicating that the HTTPServer binds to
    ...
    /**
     * Bind to this IP address. Must not be called together with {@link #hostname(String)}. Default
     * is empty, indicating that the HTTPServer binds to the wildcard address.
     */
