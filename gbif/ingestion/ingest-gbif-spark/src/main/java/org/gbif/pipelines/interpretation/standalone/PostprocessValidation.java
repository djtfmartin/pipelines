...
public class PostprocessValidation {
  ...
  //            return HdfsUtils.getLongByKey(hdfsConfigs, metaPath, m +
  // Metrics.ATTEMPTED).orElse(0L);
  ...
  //
  //    if (totalCount == 0d) {
  //      log.error("Interpreted totalCount {}, invalid absentIdCount {}", totalCount,
  // absentIdCount);
  ...
  //        validationMessage =
  //            String.format(
  //                "GBIF ID problems exceed %.0f%% threshold: %.0f%% duplicates; %d total records;
  // %d absent records",
  ...
  //        validationMessage =
  //            String.format(
  //                "GBIF ID problems within %.0f%% threshold: %.0f%% duplicates; %d total records;
  // %d absent records",
  //                threshold, absentPercent, totalCount, absentIdCount);
  //      }
  ...
  //      validationMessage =
  //          String.format("Dataset has no API records, but %.0f%% of IDs aren't new",
  // absentPercent);
  //      isValid = false;
  //    }
  //    return IdentifierValidationResult.create(totalCount, absentIdCount, isValid,
  // validationMessage);
  //  }
  //
  //  @SneakyThrows
