...
public class GeocodeKVStoreFactory {
   ...
   */
  public static Function<Result, GeocodeResponse> resultMapper(
  ...
  /**
   * Creates a mutator function that maps a key and a list of {@link GeocodeResponse.Location} into
   * a {@link Put}.
   *
   * @param columnFamily HBase column in which values are stored
   ...
   */
  public static BiFunction<byte[], GeocodeResponse, Put> valueMutator(
      byte[] columnFamily, byte[] jsonColumnQualifier) {
   ...
   */
  public static KeyValueStore<GeocodeRequest, GeocodeResponse> simpleGeocodeKVStore(
      CachedHBaseKVStoreConfiguration configuration, ClientConfiguration geocodeClientConfiguration)
      throws IOException {

    GeocodeService geocodeService =
        RestClientFactory.createGeocodeService(geocodeClientConfiguration);
    return simpleGeocodeKVStore(configuration, geocodeService, () -> {});
  }

  public static KeyValueStore<GeocodeRequest, GeocodeResponse> simpleGeocodeKVStore(
      CachedHBaseKVStoreConfiguration configuration,
      GeocodeService geocodeService,
      Command closeHandler)
      throws IOException {
    KeyValueStore<GeocodeRequest, GeocodeResponse> keyValueStore =
        Objects.nonNull(configuration.getHBaseKVStoreConfiguration())
            ? hbaseKVStore(configuration, geocodeService, closeHandler)
            : restKVStore(geocodeService, closeHandler);
  ...
  public static KeyValueStore<GeocodeRequest, GeocodeResponse> simpleGeocodeKVStore(
    ...
    KeyValueStore<GeocodeRequest, GeocodeResponse> keyValueStore =
        restKVStore(geocodeService, () -> {});
    if (Objects.nonNull(clientConfiguration.getFileCacheMaxSizeMb())) {
      return KeyValueCache.cache(
          keyValueStore,
          clientConfiguration.getFileCacheMaxSizeMb(),
          GeocodeRequest.class,
          GeocodeResponse.class);
    }
    return keyValueStore;
  ...
  public static KeyValueStore<GeocodeRequest, GeocodeResponse> simpleGeocodeKVStore(
      CachedHBaseKVStoreConfiguration configuration) throws IOException {
    KeyValueStore<GeocodeRequest, GeocodeResponse> keyValueStore =
        HBaseStore.<GeocodeRequest, GeocodeResponse, GeocodeResponse>builder()
  ...
  private static KeyValueStore<GeocodeRequest, GeocodeResponse> hbaseKVStore(
      CachedHBaseKVStoreConfiguration configuration,
      GeocodeService geocodeService,
      Command closeHandler)
      throws IOException {
  ...
  private static KeyValueStore<GeocodeRequest, GeocodeResponse> restKVStore(
      GeocodeService geocodeService, Command closeHandler) {
