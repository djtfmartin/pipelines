...
import org.gbif.pipelines.model.Audubon;
...
public class AudubonInterpreter {
  ...
  private final Function<Supplier<Audubon>, TargetHandler<Audubon>> HANDLER =
      mft ->
          ExtensionInterpretation.extension(Extension.AUDUBON)
              ...
              .map(
                  AcTerm.associatedObservationReference, Audubon::setAssociatedObservationReference)
              ...
              .map(
                  ...
                  AudubonInterpreter::parseAndSetFurtherInformationUrl)
   ...
   * interpreted values.
   */
  public void interpret(ExtendedRecord er, AudubonRecord ar, Supplier<Audubon> supplier) {
    Objects.requireNonNull(er);
    Objects.requireNonNull(ar);

    Result<Audubon> result = HANDLER.apply(supplier).convert(er);
