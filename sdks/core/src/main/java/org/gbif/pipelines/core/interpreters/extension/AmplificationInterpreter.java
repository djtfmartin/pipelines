...
public class AmplificationInterpreter {
  ...
  private static final Function<Supplier<Amplification>, TargetHandler<Amplification>> HANDLER =
      mft ->
          ExtensionInterpretation.extension(EXTENSION_ROW_TYPE)
              ...
              .map(
                  GGBN + "amplificationSuccessDetails",
                  Amplification::setAmplificationSuccessDetails)
              ...
              .map(
                  GGBN + "primerReferenceLinkForward", Amplification::setPrimerReferenceLinkForward)
              ...
              .map(
                  GGBN + "primerReferenceLinkReverse", Amplification::setPrimerReferenceLinkReverse)
  ...
  public static BiConsumer<ExtendedRecord, AmplificationRecord> interpret(
      ...
      Supplier<Amplification> amplificationSupplier,
    ...
    return (er, ar) -> {
      if (client != null) {
        ...
        Result<Amplification> result = HANDLER.apply(amplificationSupplier).convert(er);

        List<Amplification> amplifications = result.getList();
        parseAndSetBlast(amplifications, client, blastResultSupplier);
